...
function register_theme_blocks()
{
    ...
    if (!empty($blocks)) {
        ...
        foreach ($blocks as $block) {
            ...
            if ($block_settings["category"] == "neatbds_hero") {
...
/**
 * Add a block category for all neatbds ACF blocks.
 */
add_filter(
 ...
 */
add_filter('allowed_block_types_all', 'neatbds_allowed_block_types');
function neatbds_allowed_block_types($allowed_blocks)
{
    global $customblocks;
    $ret = array(
    'core/paragraph',
 ...
 */
function neatbds_block_editor_styles()
{
    wp_enqueue_style('neatbds-editor-styles', get_theme_file_uri('/css/editor.css'), [], '1.0', 'all');
}
add_action('admin_enqueue_scripts', 'neatbds_block_editor_styles');
...
add_action('neatbds_pre_render_block', 'pre_render_block_cb', 10, 1);
function pre_render_block_cb($block)
{
    ...
    if ((isset($block['data']['spacing_desktop']) AND $block['data']['spacing_desktop'] != '')  
        OR (isset($block['data']['spacing_tablet']) AND $block['data']['spacing_tablet'] != '')  
        OR (isset($block['data']['spacing_mobile']) AND $block['data']['spacing_mobile'] != '') 
    ) {
            
        $style = '<style>#'.$block['id'].'{';

        if (isset($block['data']['spacing_desktop']) AND $block['data']['spacing_desktop'] != "" ) {
            $style .= '--blockMargin: '.$block['data']['spacing_desktop'].'px;';
        }

        if (isset($block['data']['spacing_tablet']) AND $block['data']['spacing_tablet'] != "" ) {
            $style .= '--blockMarginTablet: '.$block['data']['spacing_tablet'].'px;';
        }

        if (isset($block['data']['spacing_mobile']) AND $block['data']['spacing_mobile'] != "" ) {
            $style .= '--blockMarginMobile: '.$block['data']['spacing_mobile'].'px;';
        }

        $style .= '}</style>';
        add_action(
            'wp_footer', function ( ) use ( $style ) {
                echo $style; 
            }, 100, 1
        );
    }
    ...
    if (file_exists($jsfile)) {
        ...
        if ($jsscript) {
            ...
            add_action(
                'wp_footer', function () use ($jsscript) {
                    echo $jsscript;
                }, 100, 1
            );
        }
